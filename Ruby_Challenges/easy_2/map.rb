...
# the associated block to perform some sort of operation on each collection member. 
...
# block, it obtains just one value. That value may be a complex value - it may even 
# e another collection - but it is nevertheless just one value. Thus, #map returns 
# n Array that has the same number of elements that the original collection had.
...
collection.each_with_object(collection.class.new) do |el, new_collection| 
  new_collection << yield(el) 
end
