...
# is usually appended to a partial number to generate the full number.
...
#   modulo 10 is congruent to 0), then the number is valid according to
...
# A checksum has to do with error-detection. There are a number of
# different ways in which a checksum could be calculated.

# When transmitting data, you might also send along a checksum that
# says how many bytes of data are being sent. That means that when
# the data arrives on the other side, you can count the bytes and
...
# only that it's a plausible one. It's the same thing with the bytes
# that get transmitted -- you could have the right number of bytes 
# and still have a garbled message. So checksums are a simple sanity-check,
# not a real in-depth verification of the authenticity of some data.
# It's often a cheap first pass, and can be used to quickly discard 
# obviously invalid things.

class Luhn
  attr_accessor :array_digits

  def initialize(num)
    @num = num
    @array_digits = @num.digits
  end

  def addends
    @array_digits.map.with_index do |int, idx|
      if idx.odd?
        more_than_ten?(int) ? (int * 2) - 9 : int * 2
      else
        int
      end
    end.reverse
  ...
  def self.create(num)
    luhn = new(num)
    luhn_digits = luhn.array_digits.unshift(0)

    until luhn.addends.sum % 10 == 0
      luhn_digits[0] += 1
    end

    luhn_digits.reverse.join.to_i
  end
