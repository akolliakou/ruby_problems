...
# and the Array#sort method, which expects a block argument to express how the Array 
# ill be sorted. If we want to use comparator to sort the Array, we have a problem: 
# it is a proc, not a block. The following code:
...
# to call the #to_proc method on the object first. Used with symbols, e.g., &:to_s, 
# Ruby creates a proc that calls the #to_s method on a passed object, and then 
# converts that proc to a block. This is the "symbol to proc" operation 
...
# block, & applied to a method parameter causes the associated object to be 
...
# make your modifications, and make sure to review the "Approach/Algorithm" section 
